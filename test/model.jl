      ...
      @testset "Two processes, one time step, two products" begin
        ...
        @testset "Flows" begin
          ...
          @testset "Fixed flow" begin
            ...
            postConstraints(m, eq1m, hrm)
            postConstraints(m, eq2m, hrm)
            postConstraints(m, FlowModel(m, inEquipment, e1, t, ob, maxValue=150.), eqms)
            postConstraints(m, FlowModel(m, e1, e2, t, ob, maxValue=150.), eqms)
            postConstraints(m, FlowModel(m, e2, outEquipment, t, ob, maxValue=150.), eqms)
