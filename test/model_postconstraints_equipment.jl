@testset "Each piece of equipment" begin
  ...
  @testset "Two processes, one time step, two products" begin
    date = DateTime(2017, 01, 01, 12, 32, 42)
    t = Timing(timeBeginning=date, timeHorizon=Week(1), timeStepDuration=Hour(1))
    t = Timing(timeBeginning=date, timeHorizon=Hour(4), timeStepDuration=Hour(1))
    s = Shifts(t, date, Hour(4))
    ...
    @testset "Flows" begin
      ...
      @testset "In and out flows with transformation rate" begin
        ...
        @constraint(m, sum(flowOut(eq1m, 1, p) for p in [1, 2]) >= 10)
        @constraint(m, sum(flowOut(eq1m, 2, p) for p in [1, 2]) >= 10)
        @constraint(m, sum(flowOut(eq1m, 3, p) for p in [1, 2]) >= 10)
        @constraint(m, sum(flowOut(eq2m, 4, p) for p in [1, 2]) >= 10)
        # @constraint(m, sum(flowIn(outm, d, p1) + flowIn(outm, d, p2) for d in eachTimeStep(hrm)) >= 3 * 155 * .99 * .99 * .98)
        writeLP(m, "f:/m.lp", genericnames=false)

        ## Major problem: generates a constraint flowOut(eq, t0) = 0, which is no more valid since the rewriting of the state equations (can output directly at t=t0 for very short processes). 
        ...
        @test getvalue(sum(flowIn(outm, d, p1) + flowIn(outm, d, p2) for d in eachTimeStep(hrm))) ≈ 15 * 155. * (.99 ^ 2) atol=1.e-4 
        @test getvalue(sum(flowOut(inm, d, p1) + flowOut(inm, d, p2) for d in eachTimeStep(hrm))) ≈ 15 * 155. atol=1.e-4
        println(getvalue(sum(flowOut(inm, d, p1) + flowOut(inm, d, p2) for d in eachTimeStep(hrm))))
        println("len each time step")
        println(length(eachTimeStep(hrm)))
      end
