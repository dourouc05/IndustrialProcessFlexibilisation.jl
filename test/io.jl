@testset "Input-output" begin
  @testset "Order book" begin
    @testset "As matrix" begin
      ...
      t1 = Timing(timeBeginning=date, timeHorizon=Week(2), timeStepDuration=Hour(1), shiftBeginning=date, shiftDuration=Hour(24))
      ...
      obm = convert(Array, ob, t1, shiftly=true, restrictToTiming=true)
      ...
      # The two previous test cases should not be affected by restrictToTiming.
      @test(convert(Array, ob, t1, shiftly=true, restrictToTiming=true) ≈ convert(Array, ob, t1, shiftly=true, restrictToTiming=false))
      @test(convert(Array, ob, t2, shiftly=true, restrictToTiming=true) ≈ convert(Array, ob, t2, shiftly=true, restrictToTiming=false))
      ...
      obm = convert(Array, ob, t1, shiftly=true, restrictToTiming=false)
      @test(size(obm, 1) == 14) # 2 weeks, 1-day shifts.
      @test(size(obm, 2) == 2) # Products.
      @test(sum(obm, 1) ≈ [50., 50.]')
      @test(sum(obm, 2) ≈ [50., 0., 0., 0., 0., 0., 50., 0., 0., 0., 0., 0., 0., 0.])
